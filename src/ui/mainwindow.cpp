...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    if (auto button = ui->profilesTableView->findChild<QAbstractButton *>(QString(), Qt::FindDirectChildrenOnly)) {
        // Corner Button
    ...
    if (!is_offscreen) {
        ...
        if (tray) tray->setVisible(!Configs::dataStore->disable_tray);
...
void MainWindow::show_group(int gid) {
    ...
    if (group->manually_column_width) {
        for (int i = 0; i <= 4; i++) {
            ui->profilesTableView->horizontalHeader()->setSectionResizeMode(i, QHeaderView::Interactive);
            auto size = group->column_width.value(i);
            if (size <= 0) size = ui->profilesTableView->horizontalHeader()->defaultSectionSize();
            ui->profilesTableView->horizontalHeader()->resizeSection(i, size);
        }
    } else {
...
/**
 * @brief Periodically refreshes the UI state, traffic statistics, and window/tray icons.
 * This function is heavily guarded against segmentation faults in headless environments.
 * 
 * @param traffic_update Optional string with new traffic data (e.g., from TrafficLooper).
 */
void MainWindow::refresh_status(const QString &traffic_update) {
    if (!Configs::dataStore || !ui) return;
    
    // 1. Headless Detection
    // Restricted environments (Distrobox, Docker, minimal VMs) often lack a functional 
    // system tray or window manager integration, which can cause hard crashes in Qt.
    static bool is_gui_stable = !QGuiApplication::platformName().contains("offscreen") && 
                                !QGuiApplication::platformName().contains("minimal");
    ...
    // Update internal cache if new data arrived from TrafficLooper
    ...
    // ── Core Running Status ──────────────────────────────────────────────────
    QString group_name;
    auto current_running = running; // Use local pointer to avoid race conditions
    if (current_running != nullptr) {
        auto group = Configs::profileManager->GetGroup(current_running->gid);
        if (group != nullptr) group_name = group->name;
    }

    // Update status label (guarded)
    if (QDateTime::currentSecsSinceEpoch() - last_test_time > 2 && ui->label_running) {
        ui->label_running->setText(current_running ? QString("[%1] %2").arg(group_name, current_running->outbound->DisplayName()).left(30) : tr("Not Running"));
    }
    
    // Inbound listener display
    if (ui->label_inbound) {
    ...
    if (ui->checkBox_VPN) ui->checkBox_VPN->setChecked(Configs::dataStore->spmode_vpn);
    if (ui->checkBox_SystemProxy) ui->checkBox_SystemProxy->setChecked(Configs::dataStore->spmode_system_proxy);
    
    // Profile selection mode hints
    if (select_mode && ui->label_running) {
    ...
    } else if (ui->label_running) {
        ui->label_running->setToolTip({});
    }

    // ── Window & Tray Title Construction ─────────────────────────────────────
    QString title_res;
    {
        QStringList tt;
        if (Configs::IsAdmin()) tt << "[Admin]";
        ...
        if (!core_version_suffix.isEmpty()) tt << core_version_suffix;
        if (!Configs::dataStore->active_routing.isEmpty() && Configs::dataStore->active_routing != "Default") {
            tt << "[" + Configs::dataStore->active_routing + "]";
        }
        if (current_running != nullptr) tt << current_running->outbound->DisplayTypeAndName() + "@" + group_name;
        title_res = tt.join(" ");
    }
    ...
    if (current_running != nullptr) {
        if (Configs::dataStore->spmode_vpn) icon_status_new = Icon::VPN;
        else if (Configs::dataStore->system_dns_set && Configs::dataStore->spmode_system_proxy) icon_status_new = Icon::SYSTEM_PROXY_DNS;
        else if (Configs::dataStore->system_dns_set) icon_status_new = Icon::DNS;
        else if (Configs::dataStore->spmode_system_proxy) icon_status_new = Icon::SYSTEM_PROXY;
        else icon_status_new = Icon::RUNNING;
    }

    // ── Final UI Update (Guarded) ──────────────────────────────────────────
    setWindowTitle(title_res);
    
    /* 
       CRITICAL SAFETY MEASURE: 
       Explicitly disabled tray/icon updates in this build context because 
       Qt 6.10.2 dynamically crashes inside libQt6Widgets when DBus is partially available 
       but the Desktop environment is restricted (Distrobox Signal 11).
    */
    /*
    if (is_gui_stable) {
        try {
            ...
            if (tray_functional && tray != nullptr) {
                tray->setToolTip(title_res);
                if (icon_status_new != icon_status) {
                    tray->setIcon(Icon::GetTrayIcon(icon_status_new));
                }
            }
        } catch (...) {
            // Silently absorb GUI initialization errors
        }
    }
    */
...
void MainWindow::refresh_proxy_list_impl(const int &id, GroupSortAction groupSortAction) {
    ui->profilesTableView->setUpdatesEnabled(false);
    if (id < 0) {
...
void MainWindow::refresh_proxy_list_impl_refresh_data(const int &id, bool stopping) {
    ...
    if (id >= 0)
    {
        ...
        {
            ui->profilesTableView->setUpdatesEnabled(true);
            return;
        }
        auto profile = Configs::profileManager->GetProfile(id);
    ...
    {
        ui->profilesTableView->blockSignals(true);
        int row = 0;
        auto profiles = filterProfilesList(currentGroup->profiles);
        ui->profilesTableView->setRowCount(profiles.count());
        for (const auto& profile : profiles) {
            refresh_table_item(row++, profile, stopping);
        }
        ui->profilesTableView->blockSignals(false);
    }
    ui->profilesTableView->setUpdatesEnabled(true);
}
...
void MainWindow::refresh_table_item(const int row, const std::shared_ptr<Configs::ProxyEntity>& profile, bool stopping)
{
    ...
    ui->profilesTableView->setVerticalHeaderItem(row, check);

    // C0: Type
    auto f = f0->clone();
    f->setText(profile->outbound->DisplayType());
    if (isRunning) f->setForeground(palette().link());
    ui->profilesTableView->setItem(row, 0, f);

    // C1: Address+Port
    f = f0->clone();
    f->setText(profile->outbound->DisplayAddress());
    if (isRunning) f->setForeground(palette().link());
    ui->profilesTableView->setItem(row, 1, f);

    // C2: Name
    f = f0->clone();
    f->setText(profile->outbound->name);
    if (isRunning) f->setForeground(palette().link());
    ui->profilesTableView->setItem(row, 2, f);
    ...
    ui->profilesTableView->setItem(row, 3, f);

    // C4: Traffic
    f = f0->clone();
    f->setText(profile->traffic_data->DisplayTraffic());
    ui->profilesTableView->setItem(row, 4, f);
}

// table菜单相关

void MainWindow::on_profilesTableView_itemDoubleClicked(QTableWidgetItem *item) {
...
void MainWindow::on_profilesTableView_customContextMenuRequested(const QPoint &pos) {
    ui->menu_server->popup(ui->profilesTableView->viewport()->mapToGlobal(pos)); // 弹出菜单
}

QList<std::shared_ptr<Configs::ProxyEntity>> MainWindow::get_now_selected_list() {
    auto items = ui->profilesTableView->selectedItems();
...
void MainWindow::HotkeyEvent(const QString &key) {
    ...
    runOnUiThread([=,this] {
        if (key == Configs::dataStore->hotkey_mainwindow) {
            if (tray) tray->activated(QSystemTrayIcon::ActivationReason::Trigger);
            else ActivateWindow(this);
