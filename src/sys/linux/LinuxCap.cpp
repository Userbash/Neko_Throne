...
static void Linux_Init_Paths() {
    ...
    if (isFlatpak) {
        ...
        qDebug() << "LinuxCap: Running inside Flatpak. host-spawn path:" << cachedHostSpawnPath;
    }
}
...
bool Linux_HavePkexec() {
    ...
    if (pkexecPath.isEmpty()) {
        qDebug() << "Linux_HavePkexec: pkexec NOT found";
        cachedResult = 0;
        return false;
    }

    cachedPkexecPath = pkexecPath;
    qDebug() << "Linux_HavePkexec: found pkexec at" << cachedPkexecPath << (isFlatpak ? "(host)" : "");
    cachedResult = 1;
    return true;
}
...
bool Linux_HaveSudo() {
    ...
    if (sudoPath.isEmpty()) {
        qDebug() << "Linux_HaveSudo: sudo NOT found";
        cachedResult = 0;
        return false;
    }

    cachedSudoPath = sudoPath;
    qDebug() << "Linux_HaveSudo: found sudo at" << cachedSudoPath << (isFlatpak ? "(host)" : "");
    cachedResult = 1;
    return true;
}
...
int Linux_Run_Command(const QString &commandName, const QString &args) {
    ...
    if (Linux_HavePkexec()) {
        ...
        qDebug() << "Linux_Run_Command: NO elevation tool found (pkexec/sudo)";
        return 32512; 
    }
    ...
    qDebug() << "Running Linux Command:" << command;
...
int Linux_Run_Privileged_Shell(const QString &shellCmd) {
    ...
    if (Linux_HavePkexec()) {
        ...
        qDebug() << "Linux_Run_Privileged_Shell: NO elevation tool found (pkexec/sudo)";
        return 32512;
    }
    ...
    qDebug() << "Running privileged shell:" << command;
    return system(command.toStdString().c_str());
}
...
QString Linux_FindCapProgsExec(const QString &name) {
    ...
    if (exec.isEmpty())
        ...
        qDebug() << "Found exec" << name << "at" << exec << (isFlatpak ? "(checked host)" : "");
...
bool Linux_IsPathNosuid(const QString &path) {
    ...
    if (!f.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qDebug() << "Linux_IsPathNosuid: cannot open mountinfo";
        return false;
    }
