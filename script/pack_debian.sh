...
# Use already bundled artifacts from deployment directory
# (populated by script/deploy_linux64.sh)
if [ ! -d "deployment/linux-amd64" ]; then
    echo "Error: deployment/linux-amd64 not found. Run script/deploy_linux64.sh first."
    exit 1
fi

# Create internal structure
mkdir -p Throne/opt/Throne
cp -r deployment/linux-amd64/* Throne/opt/Throne/
